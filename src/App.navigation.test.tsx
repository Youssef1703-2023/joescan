...
it.each(['/social-osint','/threat-map'])('redirects retired route %s to the dashboard without exposing its screen',async(path)=>{window.history.replaceState({},'',path);await enter();await screen.findByRole('heading',{name:'Dashboard content'});expect(location.pathname).toBe('/');expect(location.search).toBe('?start=1');fireEvent.click(screen.getByRole('button',{name:/^Navigate/}));fireEvent.change(screen.getByRole('searchbox'),{target:{value:'social'}});expect(screen.getByText('No paths found.')).toBeTruthy()});
