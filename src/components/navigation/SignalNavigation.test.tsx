...
it('only exposes the host-permitted destinations through both groups and search',()=>{
 ...
 const search=dialog.getByRole('searchbox');for(const text of ['admin','team','3d','social','osint','Threat map']){fireEvent.change(search,{target:{value:text}});expect(dialog.getByText('No paths found.')).toBeTruthy()}
