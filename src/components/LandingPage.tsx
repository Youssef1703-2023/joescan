...
export default function LandingPage({loading}:{onLogin:()=>void;loading:boolean}){
 ...
 const [mobile]=useState(()=>matchMedia('(pointer: coarse)').matches);
 ...
 const seek=()=>{if(v.readyState<1||!Number.isFinite(v.duration)||v.seeking||media.matches||mobile)return;if(Math.abs(v.currentTime-target)>.025)v.currentTime=target};
 ...
 <nav aria-label="Main navigation"><a className="landing-brand" href="/" aria-label="JoeScan home"><img src="/icon-192.png" alt="" width="34" height="34"/><b>JoeScan<small>A JOETECH PRODUCT</small></b></a><div className="nav-actions"><a href="#tools">Explore tools ↗︎</a><button className="login-button" disabled={loading} onClick={()=>setAuthOpen(true)}>Login ↗</button></div></nav>
 <main><section className="journey" ref={journey} aria-label="Discover JoeScan"><div className="stage">
 <video ref={video} id="journey-video" muted playsInline preload={mobile?"none":"auto"} poster="/media/joescan-signal-poster.jpg" aria-hidden="true" src={mobile?undefined:"/media/joescan-signal.mp4"} onError={()=>setFailed(true)}/>{mobile&&<MobileSignal/>}<div className="video-shade"/>
